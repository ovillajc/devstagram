...
@section('contenido')
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2">
            <img src="{{ asset('uploads/posts') . '/' . $post->imagen }}" alt="Imagen del post {{ $post->titulo }}">

            <div class="p-3 flex items-center gap-3">
                @auth
                    {{-- ! Funcion de like con livewire --}}
                    <livewire:like-post :post="$post" />

                    {{-- ! Funcion de like sin livewire --}}
            ...
            @auth
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{ route('posts.destoy', $post) }}" method="POST">
